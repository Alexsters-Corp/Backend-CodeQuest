...
const login = async (req, res) => {
  try {
    ...
    console.log('[Login] Hash en DB (tipo):', typeof user.password)
    console.log('[Login] Hash en DB (longitud):', user.password ? user.password.length : 'NULL')

    // bcryptjs 3.x requiere strings y hash de exactamente 60 caracteres
    const passwordStr = String(password).trim()
    const hashStr = String(user.password).trim()

    console.log('[Login] Password recibido (longitud):', passwordStr.length)
    console.log('[Login] Hash procesado (longitud):', hashStr.length)
    ...
    console.log('[Login] Comparando contraseñas con bcrypt...')
    ...
    // Actualizar ultimo_login (ignorar si la columna no existe)
    try {
      await pool.query('UPDATE usuarios SET ultimo_login = NOW() WHERE id = ?', [user.id])
    } catch (updateError) {
      console.warn('[Login] No se pudo actualizar ultimo_login:', updateError.message)
    }
